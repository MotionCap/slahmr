...
male_model_path = '/homeL/cong/HitLyn/slahmr/assets/smpl/SMPL_m_unityDoubleBlends_lbs_10_scale5_207_v1.0.0.fbx'
# female_model_path = '/homeL/cong/HitLyn/slahmr/assets/smpl/SMPL_f_unityDoubleBlends_lbs_10_scale5_207_v1.0.0.fbx'
character_model_path = None

'''
python export_fbx.py --input=/homeL/cong/HitLyn/slahmr/outputs/logs/video-val/2023-04-29/skating-all-shot-0-0--1/motion_chunks/results.npz --output=/homeL/cong/HitLyn/slahmr/outputs/logs/video-val/2023-04-29/skating-all-shot-0-0--1/motion_chunks/output.fbx --gender=male
'''
...
def process_poses(
    ...
    # embed();exit()
    data_all = np.load(input_path)
    frame_num = data_all['pose_body'].shape[1]
    pose_all = np.concatenate((data_all['root_orient'], data_all['pose_body']), axis = 2)[0] # (frame_num, 66)
    # add hand
    poses = np.concatenate((pose_all, np.zeros([frame_num, 6])), axis = 1)
    trans = data_all['trans'][0] # (frame_num, 3)
